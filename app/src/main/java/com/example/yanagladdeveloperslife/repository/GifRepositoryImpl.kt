...
class GifRepositoryImpl @Inject constructor(
    ...
    override fun getRandomGif(): Single<RandomGifViewState> =
        Single.create { emitter ->
            compositeDisposable.add(remoteDataProvider.getRandomGif()
                .observeOn(Schedulers.io())
                .subscribeOn(Schedulers.io())
                .subscribe(
                    { gifs -> emitter.onSuccess(RandomGifViewState.Loaded(gifs)) },
                    {
                        when (it) {
    ...
    override fun getLatestGifs(
        ...
        Single.create { emitter ->
            compositeDisposable.add(remoteDataProvider.getLatestGifs(page, pageSize, types)
                .observeOn(Schedulers.io())
                .subscribeOn(Schedulers.io())
                .subscribe(
                    { gifs ->
    ...
    override fun getTopGifs(
        ...
        Single.create { emitter ->
            compositeDisposable.add(remoteDataProvider.getTopGifs(page, pageSize, types)
                .observeOn(Schedulers.io())
                .subscribeOn(Schedulers.io())
                .subscribe(
                    { gifs -> emitter.onSuccess(RecyclerGifViewState.Loaded(gifs)) },
                    {
                        when (it) {
                            is IOException -> emitter.onSuccess(RecyclerGifViewState.Error.NetworkError)
                            else -> emitter.onSuccess(RecyclerGifViewState.Error.UnexpectedError)
